...
async function analyzeLegalReviewInChunks(request, options = {}, runnerConfig = getRunnerConfig()) {
  ...
  const chunkConcurrency = isKimi
    ? (Number.isFinite(envKimiConcurrency) && envKimiConcurrency > 0 ? envKimiConcurrency : 2)
...
function runConfiguredSkillCommand(request, options = {}, runnerConfig = getRunnerConfig()) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      terminateChild();
      settleReject(new Error(`Failed to write to runner stdin: ${writeError.message || writeError}`));
    }
  });
}
